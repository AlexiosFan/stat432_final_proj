...
# using pca
set.seed(123) 
sample_size <- floor(0.8 * nrow(MyTrain_pca))
...
set.seed(123) 
sample_size_rf <- floor(0.8 * nrow(MyTrain_RF))
...
plot(hc_rf)
set.seed(123)
km_result_rf <- kmeans(train_sample_rf[, -1], centers = 6)
h_clusters_rf <- cutree(hc_rf, 6)
...
set.seed(123) 
sample_size_u <- floor(0.8 * nrow(MyTrain_umap))
