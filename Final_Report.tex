...
\hline
\end{tabular}
\caption{Mean scores for each cluster from Hierarchical Clustering.}
...
\subsection{Association with Outcome Variable}
The clustering results were examined to assess their association with the outcome variable, which in this context is the essay score. Both K-Means and Hierarchical Clustering methods revealed discernible patterns that correlate with the scores assigned to the essays. The mean scores within clusters indicated a stratification effect where certain clusters were characterized by higher or lower essay scores. This suggests that the clusters may be capturing underlying features that are significant in determining the score of an essay.

\subsection{Insights from Clustering Results}
The clustering results offer valuable insights into the data structure and the relationships between different essays. For instance, essays that are grouped into the same cluster might share similar writing styles, topics, or other latent attributes that are not explicitly measured but are influential in the scoring process. These insights could guide further feature engineering and data preprocessing steps for the supervised learning models.

\subsection{Utility for Supervised Model}
...
\begin{figure}[H]
    \centering
    \includegraphics*[scale=0.25]{figures/knn_performance.png}
