...
umap_result <- umap(pca$x[,1:3], n_neighbors = 5, n_components = 2, metric = "euclidean")
plot(umap_result$layout, col = train[,1])
```
...
# plot the first two most important variables vs the target
plot(train[,15], train[,13], col = train[,1])

# umap for the first two most important variables
umap_result <- umap(train[,c(12,13,15)], n_neighbors = 5, n_components = 4, metric = "euclidean")
plot(umap_result$layout, col = train[,1])
colnames(train)
```
